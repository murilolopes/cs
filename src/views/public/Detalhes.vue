<template>
  <b-card>
    ...
    <b-row v-for="(doc, index) in docs" :key="index">
      <b-col cols="12">
        <b-card-actions class="mt-2" :title="doc.tipo" action-collapse :collapsed="index !== 0">
          <div class="border">
            <b-table
              responsive="sm"
...
export default {
  ...
  async mounted() {
    const { data } = await this.$store.dispatch('publicStore/getEmission', this.$route.params.id)
  ...
  methods: {
    async getDocs() {
      const { data } = await this.$store.dispatch('publicStore/getDocs', this.$route.params.id)
